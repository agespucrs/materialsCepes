...
CREATE TABLE TB_IDIOMA (
  ID_IDIOMA int(8) NOT NULL AUTO_INCREMENT,
  NOME varchar(120) NOT NULL,
...
CREATE TABLE TB_EQUIPAMENTOS (
  ...
  VALOR_AQUISICAO decimal(7,2) DEFAULT NULL,
  DATA_CADASTRO datetime NOT NULL,
  OBSERVACAO text,
...
CREATE TABLE TB_COMPUTADOR (
  ...
  CONSTRAINT `FK_ID_EQUIPAMENTOS` FOREIGN KEY (`ID_EQUIPAMENTO`) REFERENCES `TB_EQUIPAMENTOS` (`ID_EQUIPAMENTO`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE TB_MOBILE (
  ID_MOBILE int(11) NOT NULL AUTO_INCREMENT,
  NOME varchar(120) NOT NULL,
  ID_EQUIP_MOVEL int(11) NOT NULL,
  PRIMARY KEY (`ID_MOBILE`),
  KEY `FK_ID_EQUIP_MOVEL_idx` (`ID_EQUIP_MOVEL`),
  CONSTRAINT `FK_ID_EQUIP_MOVEL` FOREIGN KEY (`ID_EQUIP_MOVEL`) REFERENCES `TB_EQUIPAMENTOS` (`ID_EQUIPAMENTO`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE TB_PERIFERICO (
  ID_PERIFERICO int(11) NOT NULL AUTO_INCREMENT,
  NOME varchar(120) NOT NULL,
  ID_EQUIP_PERIF int(11) NOT NULL,
  PRIMARY KEY (`ID_PERIFERICO`),
  KEY `FK_ID_EQUIPAMENTO_idx` (`ID_EQUIP_PERIF`),
  CONSTRAINT `FK_ID_EQUIP_PERI` FOREIGN KEY (`ID_EQUIP_PERIF`) REFERENCES `TB_EQUIPAMENTOS` (`ID_EQUIPAMENTO`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
