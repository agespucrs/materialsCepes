...
<div class="panel panel-primary panel-add">
	...
	<div class="panel-heading text-center">Alteração de Projetos</div>
 <% Projeto projeto = (Projeto) request.getAttribute("projeto"); %>
	<div class="panel-body">

		<form action="main?acao=alterProjeto" method="post">
		<input type="hidden" id="idProjeto" name="idProjeto" value="<%=projeto.getId()%>"/>
			<jsp:include page="/template/msg.jsp"></jsp:include>

			<div class="form-group">
				<label class="form-label ages">Nome Projeto: <span
					class="red">*</span></label> <input class="form-control" id="nome"
					name="nome" type="text" value="<%=projeto.getNomeProjeto() %>" required> <label
					class="form-label ages">Programa: <span class="red">*</span></label>
				<input class="form-control" id="programa" name="programa" value="<%=projeto.getPrograma() %>" type="text" required> <label
					class="form-label ages">Origem: <span class="red">*</span></label>
				<input class="form-control" id="origem" name="origem" value="<%=projeto.getOrigem() %>" type="text" required>
				...
				<div class="row">
					<div class="col-sm-6">
						<label class="form-label ages">Coordenador Projeto </label> <select
							...
							<option value="<%=usuario.getIdUsuario() %>" <%= projeto.getIdCoordenador().equals(usuario.getIdUsuario()) ? "selected" : "" %>><%=usuario.getNome() %></option>
							<%} %>
						</select>
					</div>
				</div>
